...
const W := 1280.0
const H := 720.0
const MINE_BUTTON := Rect2(285, 480, 360, 92)
const PICKAXE_BUTTON := Rect2(850, 180, 385, 86)
const DRILL_BUTTON := Rect2(850, 285, 385, 86)
const SELL_BUTTON := Rect2(850, 390, 385, 86)
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		...
		pressed = true
	if pressed:
		if MINE_BUTTON.has_point(point):
			mine()
		elif PICKAXE_BUTTON.has_point(point):
			buy_pickaxe()
		elif DRILL_BUTTON.has_point(point):
			buy_drill()
		elif SELL_BUTTON.has_point(point):
			sell_ore()
...
func _draw() -> void:
	...
	button(PICKAXE_BUTTON, "Picareta Lv.%d" % pickaxe_level, "$%s  •  [1]" % money_text(pickaxe_cost()), coins >= pickaxe_cost())
	button(DRILL_BUTTON, "Furadeira Lv.%d" % drill_level, "$%s  •  [2]  •  +1/s" % money_text(drill_cost()), coins >= drill_cost())
	button(SELL_BUTTON, "Vender minério", "Converter tudo em créditos  •  [S]", ore > 0.0)

...
class ControllerAccessibilityService : AccessibilityService() {
    ...
    private fun axis(event: MotionEvent, axis: Int): Float =
        try { event.getAxisValue(axis) } catch (_: Exception) { 0f }
    ...
    private fun send(message: String) {
        ...
        Thread {
            try {
                ...
                socket?.send(
                    DatagramPacket(data, data.size, InetAddress.getByName(tvIp), PORT)
                )
            } catch (_: Exception) { }
        }.start()
    }

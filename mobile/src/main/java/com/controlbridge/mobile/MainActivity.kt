...
class MainActivity : Activity() {
    ...
    private var companionDevice: BluetoothDevice? = null
    ...
    private fun buildHud() {
        ...
        titleBox.addView(TextView(this).apply {
            text = "GAMEPAD BRIDGE  •  BLUETOOTH"
            textSize = 11f
            setTextColor(Color.rgb(130, 160, 205))
        })
        ...
        val pulse = TextView(this).apply {
            ...
            textSize = 12f
            typeface = Typeface.DEFAULT_BOLD
            setTextColor(Color.rgb(90, 220, 150))
            setPadding(0, 18, 0, 20)
        }
        root.addView(pulse)
        val fade = AlphaAnimation(0.45f, 1f).apply { duration = 1100; repeatCount = AlphaAnimation.INFINITE; repeatMode = AlphaAnimation.REVERSE }
        pulse.startAnimation(fade)

        root.addView(card("🎮  MANETE", "Conecte a manete ao celular primeiro. O app não ativa o HID da TV enquanto você faz isso."))
        controllerStatus = statusText("⚪ Nenhuma manete selecionada")
        root.addView(controllerStatus)

        val pair = actionButton("CONECTAR MANETE", Color.rgb(28, 92, 180))
        pair.setOnClickListener { pairController() }
        root.addView(pair)

        val accessibility = actionButton("ATIVAR CAPTURA DOS BOTÕES + ANALÓGICOS", Color.rgb(38, 45, 58))
        ...
        tvStatus = statusText("⚪ Modo TV desligado")
        root.addView(tvStatus)

        val tvButton = actionButton("ATIVAR MODO TV", Color.rgb(28, 92, 180))
        tvButton.setOnClickListener { startTvMode() }
        root.addView(tvButton)

        val tvSettings = actionButton("ABRIR CONFIGURAÇÕES BLUETOOTH", Color.rgb(38, 45, 58))
        tvSettings.setOnClickListener { startActivity(Intent(Settings.ACTION_BLUETOOTH_SETTINGS)) }
        root.addView(tvSettings)
        ...
        logText = TextView(this).apply {
            ...
            setPadding(4, 22, 4, 0)
            gravity = Gravity.CENTER
            text = "ControlBridge • sem APK na TV • baixa latência"
        }
    ...
    private fun card(title: String, body: String): LinearLayout {
        ...
        box.addView(TextView(this).apply {
            text = body
            textSize = 12f
            setTextColor(Color.rgb(165, 172, 185))
            setPadding(0, 6, 0, 0)
        })
        box.layoutParams = LinearLayout.LayoutParams(-1, -2).apply { topMargin = 12 }
        return box
    }
    ...
    private fun statusText(text: String): TextView = TextView(this).apply {
        this.text = text
        textSize = 13f
        setTextColor(Color.rgb(180, 188, 200))
    ...
    private fun pairController() {
        if (Build.VERSION.SDK_INT < 26) {
            logText.text = "Android 8+ é necessário para o pareamento assistido."
            return
        }
        try {
            ...
            companionManager.associate(request, Executors.newSingleThreadExecutor(), object : CompanionDeviceManager.Callback() {
                override fun onDeviceFound(chooserLauncher: android.content.IntentSender) {
                    runOnUiThread {
                        try {
                            startIntentSenderForResult(chooserLauncher, REQUEST_SELECT_DEVICE, null, 0, 0, 0)
                        } catch (e: Exception) {
                            logText.text = "Não foi possível abrir a seleção Bluetooth."
                        }
                    }
                }
                ...
                override fun onAssociationCreated(associationInfo: android.companion.AssociationInfo) {
                    runOnUiThread { logText.text = "Pareamento autorizado pelo Android. Conectando…" }
                }
                override fun onFailure(error: CharSequence?) {
                    runOnUiThread { logText.text = "Falha ao procurar manete: ${error ?: "nenhum dispositivo encontrado"}" }
                }
            })
            ...
            logText.text = "Não foi possível iniciar o pareamento: ${e.message ?: "erro"}"
        }
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (device == null) {
            logText.text = "O Android não retornou a manete selecionada."
            return
        }
        ...
        logText.text = "Agora ative a captura de acessibilidade. O HID da TV só será ativado quando você mandar."
    }
    ...
    private fun startTvMode() {
        ...
        tvStatus.startAnimation(TranslateAnimation(0f, 8f, 0f, 0f).apply { duration = 120; repeatCount = 3; repeatMode = TranslateAnimation.REVERSE })
    }

    private fun refreshStatus() {
        val adapter = BluetoothAdapter.getDefaultAdapter()
        if (adapter == null) {
            logText.text = "Este celular não possui Bluetooth."
            return
        }
        if (Build.VERSION.SDK_INT >= 31 && checkSelfPermission(Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED) {
            logText.text = "Conceda a permissão ‘Dispositivos próximos’."
        }
    }

    companion object {
        private const val REQUEST_SELECT_DEVICE = 401
    }
}
